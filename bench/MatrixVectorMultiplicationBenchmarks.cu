...
BENCHMARK_REGISTER_F(GeneralFixture, CPUMatVecMult)->ArgsProduct({{0, 1, 2, 3}, {0}})->UseManualTime();
BENCHMARK_REGISTER_F(GeneralFixture, GPUMatVecMultScalar)
    ->ArgsProduct({PARAMETERS, {0}, BATCH_CONFIG, {0}})
    ->UseManualTime();
BENCHMARK_REGISTER_F(GeneralFixture, CPUMatVecMultScalar)->ArgsProduct({{0, 1, 2, 3}, {0}})->UseManualTime();
